...
void WindActorWrapper::createData() {
        ...
        for (int j = 0; j < data_axis_y; ++j) {
            for (int k = 0; k < data_axis_z; ++k) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    double y = j + (rand() % 20) * 0.1;
                    double z = k - (rand() % 20) * 0.05;
    ...
    for (int l = 0; l < colorNumber; ++l) {
        // 使用 jet256
        float *pDouble = Jet256ColorMap::colormap[l];
        // 随距离不一样，Alpha不渐变
        lut->SetTableValue(l, pDouble[0], pDouble[1], pDouble[2], float(l) / (colorNumber * 2));
    }
        ...
        for (int j = 0; j < data_axis_y; ++j) {
            for (int k = 0; k < data_axis_z; ++k) {
                ...
                for (int l = 0; l < 4; ++l) {
                    ...
                    if (rand() % 10 == 0 || (pointx > data_axis_x - 1 || pointx < 0.05) ||
                        pointz > 10) {
                        pointx = data_axis_x;
                        // z轴方向恢复
                        if (useParabola) {
                    ...
                    scalars->SetValue(id, (data_axis_x - pointx) * 2 + 20);
                }
            }
        }
    }
